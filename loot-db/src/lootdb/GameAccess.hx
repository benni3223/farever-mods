...
class GameAccess {
    ...
    /** Heaps stores the point struct as PointImpl. `h2d.col.Point` is not a constructable game type. */
    public static function point(x:Float, y:Float):Dynamic {
        var type = HlxRuntime.resolveType("h2d.col.PointImpl");
        if (type == null) return null;
        var point = HlxRuntime.allocInstance(type);
        set(point, "x", x);
        set(point, "y", y);
        return point;
    }
