...
class RiftMarkersTest {
    ...
    static function main():Void {
        ...
        G.riftEvent.state = "open";
        G.riftEvent.countdown = 494.;
        G.riftEvent.activeRift = "OtherMap";
        rifts.update(layer, 5.9);
        eq(rifts.alertActive(), true, "open portal keeps alerts active even on another map");
        eq(rifts.alertTarget(), null, "open portal on another map never redirects the arrow to a local future Rift");
        G.riftEvent.state = "closed";
        rifts.update(layer, 6.2);
        eq(rifts.alertActive(), true, "upcoming alert remains active after closure if within 15 minutes");
        eq(rifts.alertTarget().id, "First", "closure returns the arrow to an imminent upcoming Rift");

        G.riftEvent = null;
        G.randomIndex = 0;
        clock.serverNow = 3650.;
        rifts.update(layer, 6.5);
        eq(rifts.alertActive(), false, "removed event does not leave stale open portal alerts");
        eq(rifts.alertTarget(), null, "removed event does not leave a stale arrow target");
        ...
        rifts.update({_time: {_time: {serverStart: 3600., serverNow: 200.}}, worldEvents: {}}, 6.51);
